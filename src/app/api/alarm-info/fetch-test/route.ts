...
export async function POST() {
  try {
    ...
    // 只测试几个预警信息
    const testCases = [
      { typeCode: "04", typeName: "寒潮", levelCode: "01", levelName: "蓝色" },
      { typeCode: "02", typeName: "暴雨", levelCode: "02", levelName: "黄色" },
      { typeCode: "09", typeName: "雷电", levelCode: "03", levelName: "橙色" },
      { typeCode: "01", typeName: "台风", levelCode: "04", levelName: "红色" }
    ]
    
    for (const testCase of testCases) {
      const { typeCode, typeName, levelCode, levelName } = testCase
      ...
      try {
        ...
        if (!match) {
          ...
          console.warn(`响应内容: ${responseText.slice(0, 200)}`)
          failCount++
          continue
        }
    ...
    console.log(`预警信息测试获取完成: 成功 ${successCount} 条, 失败 ${failCount} 条`)
